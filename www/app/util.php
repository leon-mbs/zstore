...
function money2str_ua($money, $options = 0) {
    ...
    $numbers_m = array('', 'один', 'два', 'три', 'чотири', "п'ять", 'шість', 'сім',
    ...
    $units_ru = array(
        (($options & M2S_KOPS_SHORT) ? array('коп.', 'коп.', 'коп.') : array('копійка', 'копійки', 'копійок')),
                ...
                for ($j = 2; $j >= 0; $j--) {
                    ...
                    if ($dig != 0) {
                        ...
                        if ($j == 1 && $dig == 1) {
                            ...
                            $ret .= $numbers_m[$dig] . ' ';
                            break;
                        } // копiйки, гривнi й тисячi - жiночого роду, мiльйони й мiльярди - чоловiчого
                        elseif (($i == 2 || $i == 1 || $i == 0) && $j == 0 && ($dig == 1 || $dig == 2)) {
                            $ret .= $numbers_f[$dig] . ' ';
                        } // the main case
                        else {
                            $ret .= $numbers_m[(int)($dig * pow(10, $j))] . ' ';
...
function dec_digits_group($number, $power, $digits = 1) {
    ...
    //округлюємо перед відкиданням дробової частини: 8.29*100 у float дорівнює
    //828.99999..., через що копійки зменшувались на одну
    return  intval(round($number / pow(10, intval($power) * intval($digits)), 6)) % intval(pow(10, $digits)) ;
