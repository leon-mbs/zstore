...
class TaxInvoice extends Document
{
    ...
    public function generateReport() {
        ...
        $customer = Customer::load($this->customer_id);
        if ($customer->nds == 0) {
            $couse = "2";
            $customer->inn = "10000000000";
        }

        $header = array('date' => date('dmY', $this->document_date),
            "_sdetail" => $detail,
            "firmname" => $firm['name'],
            "firmcode" => \App\Util::addSpaces($firm['inn']),
            "customername" => $customer->customer_name,
            "customercode" => \App\Util::addSpaces($customer->inn),
            "saddress" => $firm['street'] . ',' . $firm['city'],
            "baddress" => $customer->street . ',' . $customer->city,
            "sphone" => \App\Util::addSpaces($firm['phone']),
            "bphone" => \App\Util::addSpaces($customer->phone),
    ...
    public function exportGNAU() {
        ...
        $customer = Customer::load($this->headerdata["customer"]);
