...
class Items extends \App\Pages\Base
{
    ...
    public function filterOnSubmit($sender)
    {
        ...
        if ($data['error'] == "") {
            ...
            foreach($items as $item){
                ...
                if(in_array($item->item_code,$data['articles'])) continue; //уже  в  магазине
                if(strlen($item->qty)==0)$item->qty=0;
                $this->_items[] = $item;
            }    
    ...
    public function onGetItems($sender) {
        ...
        foreach($data['products'] as $product){
            ...
            if($cnt >0) continue; //уже  есть с  таким  аритикулом
            ...
            if($modules['ocpricetype']=='price1')$item->price1= $product['price'];
            if($modules['ocpricetype']=='price2')$item->price2= $product['price'];
            if($modules['ocpricetype']=='price3')$item->price3= $product['price'];
            if($modules['ocpricetype']=='price4')$item->price4= $product['price'];
            if($modules['ocpricetype']=='price5')$item->price5= $product['price'];
            $item->save();
            $i++;
        }
