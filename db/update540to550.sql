...
AS
 ...
 INSERT INTO `metadata` ( `meta_type`, `description`, `meta_name`, `menugroup`, `disabled`) VALUES( 3, 'ѕр. процессы', 'ProdProcList', 'ѕроизводство', 0);
 INSERT INTO `metadata` ( `meta_type`, `description`, `meta_name`, `menugroup`, `disabled`) VALUES( 3, 'ѕр. этапы', 'ProdProcList', 'ѕроизводство', 0);
 
 
  
журнал процесов, редактирование  этапов
заказ  если  есть или договор
создаем  процесс с нул€  или  на  основании  копии
старт  процесса. отмена  пока нет документов

отчет по процессу сколько  списано  оприходовано  нормочасы или  сдельна€
  

производственный цикл - процес плюс дата процесса
дата  определ€етс€  этапми 
в журнале  редактируетс€  список  этапов
...
создание  списани€  и оприходовани€ - прив€зка  документов  к  этапу
инфа об этапе  в  комент
подсчет по  докам сколько  списано  оприходовано и сколько надо
исполнители  с  коефициентами (в  нар€д такое?)
